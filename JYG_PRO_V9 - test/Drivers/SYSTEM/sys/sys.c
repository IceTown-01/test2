 ...
 *              pll2_p_ck = (25 / 5) * 88 / 2) = 220Mhz (25MHz晶振配置)
 *              pll2_r_ck = FMC时钟频率 = ((25 / 5) * 88 / 2) = 220Mhz (25MHz晶振配置)
 *
...
uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
     ...
     *  配置SPI1/2/3时钟源为PLL2P (220MHz)，适合SPI外设使用
     */
    rcc_periph_clk_init.PeriphClockSelection = RCC_PERIPHCLK_USART2 | RCC_PERIPHCLK_USART1
                                               | RCC_PERIPHCLK_USB 
                                               | RCC_PERIPHCLK_FMC
                                               | RCC_PERIPHCLK_SPI123;
    /* PLL2配置：适配25MHz晶振，输出220MHz
     * PLL2输出 = (HSE / PLL2M) * PLL2N / PLL2P = (25 / 5) * 88 / 2 = 220MHz
     */
    rcc_periph_clk_init.PLL2.PLL2M = 5;   /* 25MHz晶振：预分频5 */
    rcc_periph_clk_init.PLL2.PLL2N = 88;  /* 倍频88 (从88改为适配25MHz) */
    rcc_periph_clk_init.PLL2.PLL2P = 2;   /* 后分频2 */
    ...
    rcc_periph_clk_init.Spi123ClockSelection = RCC_SPI123CLKSOURCE_PLL2;  /* SPI1/2/3使用PLL2P (220MHz) */
